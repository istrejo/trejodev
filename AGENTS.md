...
Split template and styles into separate files when the component is large or complex. Keep everything inline only for genuinely trivial atoms.

**Rule: use separate files when ANY of these are true:**
- Template exceeds ~10 lines of HTML
- Styles define more than 2–3 rules
- Component is a page, layout shell, or molecule with real logic

**Rule: inline template/styles are OK only when ALL of these are true:**
- Atom with a single element or a trivial wrapper (e.g. a `<button>`, `<span>`, `<a>`)
- Template fits in 1–3 lines without wrapping
- No custom SCSS needed (Tailwind classes only)

```ts
// Separate files — pages, layout, complex molecules
@Component({
  selector: 'app-home',
  standalone: true,
...
// Inline — simple atom, single element
@Component({
  selector: 'app-badge',
  standalone: true,
  template: `<span class="font-mono text-xs px-2 py-0.5 bg-surface text-muted"><ng-content /></span>`,
})
```

When in doubt, split. Inline templates make git diffs noisy and editors lose HTML tooling.

---

## Styling rules

- Component styles: `styleUrl` (external SCSS) for non-trivial components; inline `styles` only for single-rule atoms.
